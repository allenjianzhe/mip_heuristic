#file name netflow042015MIP.py, based on netflow0814MIP.py, try to remove all the time constraint.
import sys
                ...
                for k in modes:
                        for s in range(len(departure)):
                                if X[int(row[0]),0,1,k,s].x > 0:
                                        print 'Customer',int(row[0]),'arc','0-1','mode',k,'departure time',s+1,'start_Time',dT[0,1,k,s],'Ending_time',t[int(row[0]),4].x,'Tardiness'#,T[int(row[0])].x
                 ...
                 for k in modes:
                        for s in range(len(departure)):
                                if X[int(row[0]),i,i+1,k,s].x > 0:
                                        print 'Customer',int(row[0]),'arc',i,i+1,'arc_mode_num',k,'departureNum',s+1,'arc_cost',arc_trans_cost[int(row[0]),i,i+1,k],'start_Time',dT[i,i+1,k,s],'Trans_time',trans_time[i,i+1,k],'linkTransportCost',arc_trans_cost[int(row[0]),i,i+1,k]
