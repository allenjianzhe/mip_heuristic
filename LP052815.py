#file name LP052815.py  based on 052815.py
import sys
...
if m.status == GRB.status.OPTIMAL:
        for row in customer:
            print 'Customer',int(row[0]),'DEMAND',int(row[2]),'DD',DD[int(row[0])],'Tardiness',T[int(row[0])].x
            ...
            for i in nodes:
                if i==3:
                    ...
                    if y2[int(row[0]),i].x==1:
                        #print 'row',int(row[0]),'i',i,'y',y2[int(row[0]),i].x
                ...
                for k in modes:
                    for s in departure:
                            if X2[int(row[0]),i,k,s].x > 0:
                                #print 'row',int(row[0]),'i',i,'k',k,'s',s,'X2',X2[int(row[0]),i,k,s].x
