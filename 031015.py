#031015.py 可以运行，但要注意capacity的大小于用户demand不能差太多。
import sys
    ...
    if len(node_arc)>0:
      ...
      for iks in range(len(node_arcs)):
          ...
          if p<=arc_C[0,1,k_try,s_try]:
              ...
              #back to original, meanwhile we should let this new varilable back to 0 so that next time we would test again.
              example1.append(a)
              a=[]
              rr=r_try
              max_value=[]
            ...
            for k in modes:
                for s in range(len(departure)):
                    if X1[int(row[0]),k,s].x ==1:
                        print 'Customer',int(row[0]),'arc',0,1,'arc_mode_num',k,'departureNum',s+1,'X1',X1[int(row[0]),k,s].x,'StartTime',dT[0,1,k,s],'TransTime',trans_time[0,1,k],'endingTime',t[int(row[0]),1].x
            ...
            for k in modes:
                for s in range(len(departure)):
                    if X2[int(row[0]),k,s].x ==1:
                        print 'Customer',int(row[0]),'arc',1,2,'arc_mode_num',k,'departureNum',s+1,'X2',X2[int(row[0]),k,s].x,'StartTime',dT[1,2,k,s],'TransTime',trans_time[0,1,k],'endingTime',t[int(row[0]),2].x,'Tardiness',T[int(row[0])].x
