#netflow0322MIP.py  based on file name netflow0814MIP.py
import sys
                 ...
                 for k in modes:
                        for s in range(len(departure)):
                                if X[int(row[0]),i,i+1,k,s].x > 0:
                                        print 'Customer',int(row[0]),'arc',i,i+1,'arc_mode_num',k,'departureNum',s+1,'arc_cost',arc_trans_cost[int(row[0]),i,i+1,k],'start_Time',dT[i,i+1,k,s],'Trans_time',trans_time[i,i+1,k],'Ending_Time',t[int(row[0]),i+1].x,'linkTransportCost',arc_trans_cost[int(row[0]),i,i+1,k]
