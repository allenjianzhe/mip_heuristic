...
TotalCostList=[]
FinalTotalCost=25000000
for drift in xrange(-5, 6):
    #print '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$','drfit',drift
    from read061215 import *
    m = Model('MIP')
    M=1000000000
    F=600000
    y={}
            ...
            for k in modes:
                for s in departure:
                    if X2[int(row[0]),2,k,s]>0:
                        if t[int(row[0]),2]>DD[int(row[0])]+drift:
                            m2.addConstr(T[int(row[0])],GRB.EQUAL,t[int(row[0]),2]-DD[int(row[0])]+drift,name='Tardiness_%s'%(int(row[0])) )
        m2.update()
        m2.__data=X2,y2,t,T
        return m2
    ####################################################################################################
