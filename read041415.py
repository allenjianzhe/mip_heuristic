...
for row in customer:
    for i in node:
        if i==0:
            ...
            for k in modes:
                if k==1:
                    arc_trans_cost[int(row[0]),i-1,i,k]=.3*int(row[2])*Distance[i-1,i]/1000
                elif k==2:
                    arc_trans_cost[int(row[0]),i-1,i,k]=.5*int(row[2])*Distance[i-1,i]/1000
                elif k==3:
                    arc_trans_cost[int(row[0]),i-1,i,k]=1.5*int(row[2])*Distance[i-1,i]/1000
                elif k==4:
                    arc_trans_cost[int(row[0]),i-1,i,k]=2*int(row[2])*Distance[i-1,i]/1000
