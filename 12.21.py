#file name 1221.py  在11.27(一个一个的减)的基础上试着改，改constraint, 不删customer id， 变成在mip之前先对X做判断。 这样每次出来的结果都是最后的值， total_cost不断变小。  
import sys
...
##                        if (row,i,k,s) in example:
    ...
    for (row,i,k,s) in example:
        print row,i,k,s ,'This X2 is 1'
        #print X2[row,i-1,i,k,s]
        m.addConstr(X2[int(row),1,2,k,s], GRB.EQUAL, 1,name='One_%s3'%(int(row)))
    ...
    for k in modes:
        for s in range(len(departure)):
            expr = LinExpr()
            for row in customer:
                expr.addTerms(int(row[2]),X1[int(row[0]),0,1,k,s])
            expr.addConstant(-1*arc_C[0,1,k,s])
            m.addConstr(expr,GRB.LESS_EQUAL, 0,'arcCapacity1_%s_%s_%s_%s'%(int(row[0]),i,k,s))      
    m.update()
    for k in modes:
        for s in range(len(departure)):
            expr = LinExpr()
            for row in customer:
                expr.addTerms(int(row[2]),X2[int(row[0]),1,2,k,s])
            expr.addConstant(-1*arc_C[1,2,k,s])
            m.addConstr(expr,GRB.LESS_EQUAL, 0,'arcCapacity2_%s_%s_%s_%s'%(int(row[0]),i,k,s))       
    m.update()
...
#之前已经fixed为1的不用参加下面的排序！
    ...
    ####################################################################################################
      for row in max_value: 
          ...
          (r_try,j_try,i_try,k_try,s_try)=map(int,rows)
          print '(r_try,j_try,i_try,k_try,s_try)',(r_try,j_try,i_try,k_try,s_try)
          a=[]
          a.append(r_try)
          a.append(i_try)
          a.append(k_try)
...
##         check_capacity(r_try2,try2
      MIP(m,customer,arc_C,r_try,i_try,k_try,s_try)
      m.optimize()
      X1,X2,y,t,T=m.__data
