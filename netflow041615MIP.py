#file name netflow041615MIP.py based on netflow1028MIP  try to fix t problem. t is no more variables. 
import sys
        ...
        for s in range(len(departure)):
            for i in node:
                    if i==0:
                            ...
                            expr = LinExpr()
                            for row in customer:
                                    expr.addTerms(int(row[2]),X[int(row[0]),i-1,i,k,s])
    ...
    for row in customer:        
            for i in node:
                    ...
                    if i==0:
                            ...
                            m.addConstr(expr,GRB.EQUAL,t[int(row[0]),i],name='timeConstr_%s_%s'%(int(row[0]),i))                        
    m.update()                    
...
if m.status == GRB.status.INFEASIBLE:
        m.computeIIS()
        m.write('mipmodelconstraintError_2.ilp')
                ...
                for k in modes:
                        for s in range(len(departure)):
                                if X[int(row[0]),0,1,k,s].x > 0:
                                        print 'Customer',int(row[0]),'start_Time',dT[0,1,k,s],'Ending_time',t[int(row[0]),2].x,'Tardiness',T[int(row[0])].x
