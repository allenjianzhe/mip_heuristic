...
def MIP(customer,nodes,modes,departure,pi):
    ...
    m.update()
    m.optimize()
    return m,X,y,t,T,m.objVal,expr1,expr2,expr3,expr4
def MIP_OneCustomer(arc_C,kkk,pi):
    ...
    for i in nodes:                    
            ...
            if i==1 or i==4:
                    ...
                    for k in modes:
                            for s in departure:                                            
                                    expr.addTerms(dT[i,k,s],X[int(row[0]),i,k,s])
                    expr.add(y[int(row[0]),i])
                    m.addConstr(expr,GRB.GREATER_EQUAL,t[int(row[0]),i-1],name='timeConstr2_%s_%s'%(int(row[0]),i))
    m.update()
    #definition of T
    for k in modes:
        for s in departure:
...
def expr3_v(y,kkk):
    ...
    for i in nodes:
        if i==4:
            ...
            expr3=expr3+y[int(row[0]),i].x*int(row[2])*P
    return expr3
def expr4_value(T):
    expr4=0
    for row in customer:
