        ...
        for s in departure:
            for n in nodes:
                if n==4:
                    continue
                if -fix_C<GG[n,k,s]<=0:
                    print GG[n,k,s],'GG[n,k,s]',n,k,s,'-400<GG[n,k,s]<=0'
                    for row in customer:
                        if X[int(row[0]),n,k,s].x>0:
                            a=[]
                            a.append(int(row[0]))
...
##                        if X[int(row[0]),n,k,s].x>0:
##                            print X[int(row[0]),n,k,s].x,'X[int(row[0]),n,k,s].x','int(row[0]),n,k,s',int(row[0]),n,k,s
                    customers_for_current_capacity = [(int(row[0]), int(row[2])) for row in customer if X[int(row[0]),n,k,s].x]
