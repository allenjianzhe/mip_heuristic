...
ST={}#shipping time of route n if 3PL is used
...
def MIP(m,customer,arc_C,nodes,modes,departure,pi):  #建立优化模型
    ...
    #for row in customer:
        #for i in nodes:
                ...
                #continue
            #else:
                #ST[i]=m.addvar(vtype='C',name='ThreeSTime_%s_%s'%(i))
    #m.update()
...
while o!=5:# means when o=3, stop
    ...
    alpha=sigma*(Zub-Zlb)/(arc_C*arc_C) #alpha是公式26里面的（delta）δ ，sigma是λ（lamda）的表示,公式36里面求δ ，
    #alpha=alpha/5.0
