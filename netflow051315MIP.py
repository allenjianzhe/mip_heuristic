...
def MIP(m,customer,arc_C):
            ...
            if i==4:
                ...
                for k in modes:
                    for s in departure:
                        ...
                        #X[int(row[0]),i,k,s]=m.addVar(obj=arc_trans_cost[int(row[0]),i,k],vtype=GRB.BINARY,name='X_%s_%s_%s_%s'%(int(row[0]),i,k,s))
    m.update()                                
    ...
    #decision variable:Time tardiness of customer, formula 2
    T={}
    for row in customer:
...
print '\n'
for row in customer:
    print 'Customer',int(row[0])+1,'Destination ',row[1], 'quantity ',int(row[2]),'DD',int(row[3]),'penalty',int(row[4]),'Tardiness',T[int(row[0])].x
print "\n"
                ...
                for k in modes:
                    for s in departure:
                            if X[int(row[0]),i,k,s].x > 0:
                                    ...
                                    #TransCost+=arc_trans_cost[int(row[0]),i,k]
