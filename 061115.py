#file name 061115.py  based on 052715
import sys
        ...
        for row in customer:
            for i in nodes:
                if i == 2:
                    ...
                    for k in modes:
                        for s in departure:
                            X[int(row[0]),i,k,s]=m.addVar(lb=0,ub=1,obj=int(row[2])*arc_trans_cost[int(row[0]),i,k],vtype=GRB.BINARY,name='X_%s_%s_%s_%s'%(int(row[0]),i,k,s))
        m.update()                                  
        #decision variable y: binary variable of 3PL
        global y
        for row in customer:
            for i in nodes:
                y[int(row[0]),i]=m.addVar(lb=0,ub=1,obj=int(row[2])*F,vtype=GRB.BINARY,name='y_%s_%s'%(int(row[0]),i))     
        m.update()
...
##    print 'Summary solution'
    ...
    print '\n'
    print 'H SOLUTION','  ARCS:',len(customer)
