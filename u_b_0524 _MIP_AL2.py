...
####################约束条件########################约束条件#########################约束条件###########
    ...
    for row in customer:
        for i in nodes:
            if i==1 or i==4:
                continue
            else: #Constraint binarty of 3PL ，y的限制条件，前一个客户的y小于后一个客户的y
                m.addConstr(y[int(row[0]),i-1],GRB.LESS_EQUAL,y[int(row[0]),i],name='one3pl_%s_%s'%(int(row[0]),i))#模型中公式26的约束
            ...
            if i==4:
                ...
                for k in modes:
                    for s in departure:                                            
                        ...
                        m.addConstr(expr7,GRB.LESS_EQUAL,t[int(row[0]),i],name='timeConstr1_%s_%s'%(int(row[0]),i))#模型公式27约束条件          
    m.update()
    ...
    for row in customer:
        for i in nodes:
            if i==1 or i==4:
                ...
                t[int(row[0]),i-1] = t[int(row[0]),i-1]-P*(1-y[int(row[0]),i])
                t[int(row[0]),i-1] = t[int(row[0]),i-1] + St 
                m.addConstr(t[int(row[0]),i-1],GRB.LESS_EQUAL,t[int(row[0]),i])
                #模型中的公式29的约束条件，有个疑问：第三方的运输时间（ST）怎么表示他,和什么有关，,name='timeConstr3_%s_%s'%(int(row[0]),i)
    m.update()
    m.optimize()
...
def Zhb(X,y,T,pi):#Heuristic for Obtaining Feasible Solution of IP    #按照期望运输量对客户的排序，已经在read11115.py里面存在了
    arc_C={}#arc_C: each arc capacity
    arc_C=getArcC(nodes,modes,departure)
    GG = 0#G=0
    for k in modes:
        for s in departure:
            for i in nodes:
                if i==4:
                    ...
                    for row in customer:
                        if X[int(row[0]),i,k,s].x>0:
                            GG = GG + int(row[2])*X[int(row[0]),i,k,s].x#print 'middle G',G, 'i,k,s',i,k,s
        ...
        for s in departure:
            for i in nodes:
                ...
                if i==4:
                    ...
                    for row in customer:
                        if X[int(row[0]),i,k,s].x>0: #print 'i,k,s',i,k,s #print 'GG[i,k,s]',GG[i,k,s]
                            GG[i,k,s]=GG[i,k,s]+int(row[2])*XX[int(row[0]),i,k,s].x#print 'GG[i,k,s]',GG[i,k,s]                         
        ...
        for i in nodes:
            if i==4:
                ...
                for k in modes:
                    for s in departure:
                            if X[int(row[0]),i,k,s].x > 0:
                                    print 'Customer',int(row[0])+1,'link',i,'arc_mode_num',k,'departureTimeIndex',s,'arc_cost',arc_trans_cost[int(row[0]),i,k]*int(row[2]),'start_Time',dT[i,k,s],'Trans_time',trans_time[i,k],'t',t[int(row[0]),i],'real_arrive_time',dT[i,k,s]+trans_time[i,k]
        ...
        for s in departure:
            for i in nodes:
                if i==4:
                    ...
                    pi[i,k,s]=max(0,pi[i,k,s]+GG[i,k,s]*alpha)
    print 'pi','\n',pi
    alpha=sigma*(Zub-Zlb)/(G*G)   #公式26里面的（delta）δ
    Zlb=max(Z,Zlb)  #修改过得
    print 'o',o,'alpha',alpha,'Zlb',Zlb,'Zub',Zub,'sigma',sigma 
    print '\n'
...
for row in customer:
    for i in nodes:
        if i==4:
            ...
            for k in modes:
                for s in departure:
                    if X[int(row[0]),i,k,s].x> 0:#修改t[int(row[0]),i].x为t[int(row[0]),i],时间20160524
                        print 'Customer',int(row[0])+1,'link',i,'arc_mode_num',k,'departureTimeIndex',s,'arc_cost',arc_trans_cost[int(row[0]),i,k]*int(row[2]),'start_Time',dT[i,k,s],'Trans_time',trans_time[i,k],'t',t[int(row[0]),i],'real_arrive_time',dT[i,k,s]+trans_time[i,k]
                        TransCost += arc_trans_cost[int(row[0]),i,k] * int(row[2])
...
while o!=5:# means when o=3, stop
                ...
                if i==4:
                    ...
                    for k in modes:
                        for s in departure:
                                if X[int(row[0]),i,k,s].x > 0:
                                    print 'Customer',int(row[0])+1,'link',i,'arc_mode_num',k,'departureTimeIndex',s,'arc_cost',arc_trans_cost[int(row[0]),i,k]*int(row[2]),'start_Time',dT[i,k,s],'Trans_time',trans_time[i,k],'t',t[int(row[0]),i],'real_arrive_time',dT[i,k,s]+trans_time[i,k]
                                    #修改t[int(row[0]),i].x为t[int(row[0]),i],时间20160524
                                    TransCost += arc_trans_cost[int(row[0]),i,k]*int(row[2])
    ...
    if len(Z_list)==3:# run 5 round to make sure Z is not decrease anymore
        if Z<min(Z_list):
            ...
            sigma=0.9*sigma#for the previous l = 3 iterations, set λ（r+1） = 0.9λ（r）;
            Z_list=[]
            Z_list.append(Z)
    else:
        Z_list.append(Z)
