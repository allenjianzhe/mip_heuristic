#file name LP061815.py  based on 061115.py
import sys
...
TotalCostList=[]
FinalTotalCost=0
for drift in xrange(-5,6):
    #print '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$','drfit',drift
    from read061115 import *
    m = Model('MIP')
    M=1000000000
    F=600000
    y={}
        ...
        for row in customer:
            for i in nodes:
                if i == 2:
                    ...
                    for k in modes:
                        for s in departure:
                            X[int(row[0]),i,k,s]=m.addVar(lb=0,ub=1,obj=int(row[2])*arc_trans_cost[int(row[0]),i,k],vtype='C',name='X_%s_%s_%s_%s'%(int(row[0]),i,k,s))
        m.update()                                  
        #decision variable y: binary variable of 3PL
        global y
        for row in customer:
            for i in nodes:
                y[int(row[0]),i]=m.addVar(lb=0,ub=1,obj=int(row[2])*F,vtype='C',name='y_%s_%s'%(int(row[0]),i))     
        m.update()
            ...
            for k in modes:
                for s in departure:
                    if X[int(row[0]),1,k,s]>0:
                        ...
                        if t[int(row[0]),1]>DD[int(row[0])]+drift:
                            m.addConstr(T[int(row[0])],GRB.EQUAL,t[int(row[0]),1]-(DD[int(row[0])]+drift),name='Tardiness_%s'%(int(row[0])) )
        m.update()
        m.__data=X,y,t,T
        return m
    ####################################################################################################
    ...
    def MIP2(m2,customer,arc_C):    
        ...
        for row in customer:
            ...
            m2.addConstr(expr,GRB.GREATER_EQUAL, arrive_time[int(row[0]),1],name='timeConstr2_%s'%(int(row[0])))
        m2.update()
        for row in customer:
            for k in modes:
                for s in departure:
...
##    print 'Summary solution'
                    ...
                    for k in modes:
                        for s in departure:
                                if X2[int(row[0]),i,k,s].x > 0:
                                        ...
                                        arrive_time[int(row[0]),i]=dT[2,k,s]+trans_time[2,k]
        ##        print '\n'
    ...
    def MIP3(m3,customer,arc_C):    
        ...
        for row in customer:
            for i in nodes:
                if i == 4:
                    ...
                    for k in modes:
                        for s in departure:
                            X3[int(row[0]),i,k,s]=m3.addVar(obj=int(row[2])*arc_trans_cost[int(row[0]),i,k],vtype='C',name='X_%s_%s_%s_%s'%(int(row[0]),i,k,s))
        m3.update()                                        
        #decision variable y: binary variable of 3PL
        y3={}
        for row in customer:
            for i in nodes:
                y3[int(row[0]),i]=m3.addVar(obj=int(row[2])*F,vtype='C',name='y_%s_%s'%(int(row[0]),i))     
        m3.update()
        #decision variable: arrive time at each node
        t={}
        for row in customer:
        ...
        m3.update()
        ####################################################################################################
        #New constraint between y2 and y3
        for row in customer:
            global y2
            if y2[int(row[0]),2].x==1:
                m2.addConstr(y3[int(row[0]),3],GRB.EQUAL,1,name='y3Constre_%s'%(int(row[0])))
        #Constraint 3.2 for each customer, each link, only one plan can be selected
        for row in customer:
            for i in nodes:
                if i==4:
        ...
        for row in customer:
            ...
            m3.addConstr(expr,GRB.GREATER_EQUAL, arrive_time[int(row[0]),2],name='timeConstr2_%s'%(int(row[0])))
        m3.update()
...
##    print 'Summary solution'
                    ...
                    for k in modes:
                        for s in departure:
                                if X3[int(row[0]),i,k,s].x > 0:
                                        ...
                                        global arrive_time
                                        arrive_time[int(row[0]),i]=dT[3,k,s]+trans_time[3,k]
    ...
    print '\n'
    print 'H SOLUTION','  CUSTOMERS:',len(customer)
    ...
    print '\n'                   
FinalTotalCost=min(TotalCostList)
print 'FinalTotalCost',FinalTotalCost
print 'TotalCostList',TotalCostList
