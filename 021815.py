#file name 021815.py  在021215基础上 
import sys
...
def MIP(m,customer,arc_C,r_bi,k_bi,s_bi):
    ...
    for k in modes:
        for s in range(len(departure)):
            expr = LinExpr()
            for row in customer:
                expr.addTerms(int(row[2]),X1[int(row[0]),k,s])# int(row[2])表示的是demand
...
#之前已经fixed为1的不用参加下面的排序！
    ...
    def test(test_k, test_s, thre):
        #node_list应该是所有的已知的
        expr = 0# 这个函数是为了检验X2是否满足capacity的条件。这里k，s可以理解为已知，不需要验证所有的k，s。
        for idx, row in enumerate(customer):
            expr+=int(row[2])*X2[int(row[0]),int(test_k),int(test_s)].x
        print expr ,'****************************************************'
        if thre > expr:# expr 是一个expression， 要optimize以后gurobi才会赋值给expr
                          return 1
        else :
                          return 0
        #return thre > expr
...
##          print 'new_node[0][7]',new_node[0][7]#,'new_node[1]',new_node[1]
          ...
          if test(new_node[0][4], \
               new_node[0][7], \
               arc_C[1, 2, int(new_node[0][4]), int(new_node[0][7])])==1:
              max_value = new_node
              print 'new_node',new_node                          
              break
      if forindex==len(node_arcs):
          print 'Not found!'
          new_node[-1]=0
##                                          X1(new_node[][],k,s)=0
##                                          y(new_node[][])=1
    #print max_value, 'MMMMMMMMMMMMMMax value'

      for row in max_value:
          print 'row',row
          rows=row[1:-1].split(',')
          (r_try,k_try,s_try)=(rows[0],rows[1],rows[2])
