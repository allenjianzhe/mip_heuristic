#122115.py based on 121615. try to test time like capacity,read041415
import sys
import math
    ...
    for row in customer:
        for i in node:                    
                ...
                if i==0 or i==2:#i=1
                        ...
                        for k in modes:
                                for s in departure:                                            
                                        expr1.addTerms(dT[i,k,s],X2[int(row[0]),k,s])                          
...
#while len(example_x2_1)+len(example_y) !=len(customer):
                ...
                for s in departure:
                    ...
                    if X2[int(row[0]),k,s].x > 0:
                        ...
                        if X2[int(row[0]),k,s].x==1 and [int(row[0]),k,s] not in example_x2_1:
                            ...
                            a=[]# then here we need to update our test_capacity
                            test_capacity[k,s]=test_capacity[k,s]-int(row[2])
                            #print 'test_capacity', test_capacity
                            for k in modes:
                                for s in departure:
                                    if X1[int(row[0]),k,s].x > 0:
                                        a=[]
    ...
    if len(node_arc)>0:
      ...
      for iks in range(len(node_arcs)):
          ...
          #print r_try,'oooooooo'
          #print int(r_try), 'ggggggggg'
          #print int(customer[int(r_try)][2]), 'kkkkkkkkkkkk'
          #print int(r_try[2]),"LLLLLLLLLL"

          if int(customer[int(r_try)][2])<=test_capacity[int(k_try),int(s_try)] and dT[2,int(k_try),int(s_try)]>=dT[1,int(k_try),int(s_try)]+trans_time[1,2,int(k_try)]:
              example_x2_1.append(a)# put the max index into example first, then we need to test capacity
              test_capacity[int(k_try),int(s_try)]=test_capacity[int(k_try),int(s_try)]-int(customer[int(r_try)][2])
              for k in modes:
                  for s in departure:
                      if X1[r_try,k,s].x>0:
                          a_x1=[]
          ...
          #no good, we should test before we put it into the example set.
            ...
            for k in modes:
                for s in departure:
                    if X1[int(row[0]),k,s].x > 0: 
                        print 'Customer',int(row[0]),'arc',0,1,'arc_mode_num',k,'departureNum',s+1,'X1',X1[int(row[0]),k,s].x
                    if X2[int(row[0]),k,s].x > 0: 
                        print 'Customer',int(row[0]),'arc',1,2,'arc_mode_num',k,'departureNum',s+1,'X1',X2[int(row[0]),k,s].x
