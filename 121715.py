#121615.py based on 041515. try to test time like capacity
import sys
import math
...
#while len(example_x2_1)+len(example_y) !=len(customer):
    ...
    if m.status == GRB.status.OPTIMAL:
        ...
        for row in customer:
            for k in modes:
                for s in range(len(departure)):
                    if X1[int(row[0]),k,s].x > 0 and [int(row[0]),k,s] not in example_x1_1: 
                        print 'Customer',int(row[0]),'arc',0,1,'arc_mode_num',k,'departureNum',s+1,'X1',X1[int(row[0]),k,s].x
                        a=[]
                        a.append(int(row[0]))
                        a.append(k)
                        a.append(s)
                        example_x1_1.append(a)
                        a=[]# first time put X1 into example
        print '\n'
        nothing=0              
        for row in customer:
            for k in modes:
                for s in range(len(departure)):
                    if X2[int(row[0]),k,s].x > 0: 
                        print 'Customer',int(row[0]),'arc',1,2,'arc_mode_num',k,'departureNum',s+1,'X2',X2[int(row[0]),k,s].x# it can be blind
                    if X2[int(row[0]),k,s].x==1 and [int(row[0]),k,s] not in example_x2_1:
                        ...
                        nothing+=1
        print '\n'
    #until here, first round is end, check y, X1, X2 whether equal 1 then put them into the example set. 
    ...
    if len(node_arc)>0:
      node_arcs=sorted(node_arc, key=lambda x: x[1],reverse=True)#x[1] means the value of X2[int(row[0]),k,s].x, here is all the X2 together join the sequence
      ...
      for iks in range(len(node_arcs)):
          ...
          (r_try,k_try,s_try)=map(int,rows)#so far, we know the test X's index which is enough for us to test
...
##          example_x2_1.append(a)# put the max index into example first, then we need to test capacity
          ...
          if p<=test_capacity[k_try,s_try]:#means satisfy
              p=0
              #next step, we need to test time constraint
              #find X1 first
              k_try_x1=1
              s_try_x1=0
              for k in modes:
                  for s in range(len(departure)):
                      if X1[r_try,k,s].x==1:
                          ...
                          if dT[1,2,k_try,s_try]>=dT[0,1,k_try_x1,s_try_x1]+trans_time[0,1,k_try_x1]:
                              break #means all capacity and time constraint are satisfied. we can fix X2 and fix X1
              #break give you to here
...
##                          a_x1.append(k)
##                          a_x1.append(s)
##                          example_x1_1.append(a_x1) # don't hurry to add X1, once time test is completely, then we add X1 to example
                          if dT[1,2,k_try,s_try]*X2[r_try,k_try,s_try].x>=(dT[0,1,k_try_x1,s_try_x1]+trans_time[0,1,k_try_x1])*X1[r_try,k_try_x1,s_try_x1].x:
                              k_try_x1=0
                              s_try_x1=0
      ...
      if max_value==[]:
          ...
          example_y.append(rr)#not finish yeah, we need to change the X1 to 0 too. remove X1 from example1.
          
          print r_try,k_try,s_try, 'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'
          for k in modes:
                  for s in range(len(departure)):
                      a=[]
                      a.append(r_try)
                      ...
                      example_x2_0.append(a)
                      if X1[r_try,k,s].x==1:
                      #print X1[r_try,k,s].x,'GGGGGGGGGGGGGGGGGGG'
                          a=[]
                          a.append(r_try)
                          a.append(k)
                          a.append(s)
                          #print example_x1_1
                          example_x1_1.remove(a)#what is wrong? didn't express clearly
