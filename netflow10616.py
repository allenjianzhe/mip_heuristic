#file name netflow10616MIP.py  based on netflow051215MIP, delete Tardiness, change 3PL to arc. 
import sys
        ...
        for k in modes:
            for s in departure:
                if X[int(row[0]),2,k,s]>0:
                    ending_time[int(row[0])]=(dT[2,k,s]+trans_time[2,k])*X[int(row[0]),2,k,s]
